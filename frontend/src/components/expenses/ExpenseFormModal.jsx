...
function ExpenseFormModal({ expense, onClose, onSaved }) {
  const { selectedDate } = useMonthStore();
  ...
  const saveExpense = async (data) => {
    try {
      ...
      if (isEditMode) {
        await api.put(`/expense-api/updateExpense/${expense._id}`, payload);
      } else {
        await api.post("/expense-api/addExpense", payload);
      }
  ...
  return (
    <div className={modalOverlay}>
      <div className={modalCard}>
        {apiError && <div className={`${errorClass} mb-4`}>{apiError}</div>}
        ...
        >
          ...
          <div>
            ...
            {errors.amount && (
              <p className={`${errorClass} mt-2`}>{errors.amount.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.category && (
              <p className={`${errorClass} mt-2`}>{errors.category.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.item && (
              <p className={`${errorClass} mt-2`}>{errors.item.message}</p>
            )}
          </div>
