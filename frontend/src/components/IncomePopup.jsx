...
function IncomePopup({ onClose, onIncomeAdded }) {
  ...
  const addIncome = async (data) => {
    try {
      ...
      if (res?.data?.payload) {
        onIncomeAdded(res.data.payload);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className={`${glassCard} w-full max-w-md p-7`}>
        {/* HEADER */}
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-slate-800">Add Income</h2>

          <p className="text-slate-500 text-sm mt-1">
            Add your monthly income to manage savings and expenses
          </p>
        </div>

        {/* FORM */}
        <form noValidate onSubmit={handleSubmit(addIncome)}>
          {/* INPUT */}
          <div>
