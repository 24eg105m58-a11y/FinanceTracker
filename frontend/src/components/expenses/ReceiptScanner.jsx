...
import { addExpense, scanReceipt } from "../../services";
...
const ReceiptScanner = ({ onSuccess }) => {
  ...
  const handleScan = async () => {
    if (!file) {
      setError("Please provide a receipt image");
    ...
    try {
      ...
      const scannedData = res.data.payload
        ? {
            suggestedExpense: res.data.payload,
          }
        : res.data;

      const expense = scannedData?.suggestedExpense;

      const hasItem = String(expense?.item || "").trim().length > 0;

      const hasAmount = String(expense?.amount || "").trim().length > 0;

      const hasValidDate =
        !expense?.expenseDate ||
        !isNaN(new Date(expense.expenseDate).getTime());

      if (!expense || (!hasItem && !hasAmount) || !hasValidDate) {
        setError("Please upload a proper receipt image");
      ...
      setError(err.response?.data?.message || "Could not scan receipt");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveAsExpense = async () => {
    try {
      ...
      await addExpense({
        item: result.suggestedExpense.item,

        amount: parseInt(result.suggestedExpense.amount || 0),

        category: result.suggestedExpense.category || "others",
    ...
    <>
      ...
      <div className={`${glassCard} h-full`}>
        ...
        <div className="flex flex-col gap-4">
          {/* FILE INPUT */}
          <div>
            <label className={labelClass}>Receipt Image</label>
        ...
        <div className={modalOverlay}>
          <div className={`${modalCard} max-w-xl`}>
            ...
            <div className="pt-6 space-y-5">
              {error && <div className={`${errorClass} mb-2`}>{error}</div>}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={result.suggestedExpense.item || ""}
                  onChange={(e) =>
                    setResult({
                      suggestedExpense: {
                        ...result.suggestedExpense,
              ...
              <div>
                ...
                <input
                  type="text"
                  inputMode="numeric"
                  value={result.suggestedExpense.amount || ""}
                  onChange={(e) =>
                    setResult({
                      suggestedExpense: {
                        ...result.suggestedExpense,
              ...
              <div>
                ...
                <select
                  value={result.suggestedExpense.category || "others"}
                  onChange={(e) =>
                    setResult({
                      suggestedExpense: {
                        ...result.suggestedExpense,
              ...
              <div>
                ...
                <input
                  ...
                  value={
                    result.suggestedExpense.expenseDate
                      ? new Date(result.suggestedExpense.expenseDate)
                          .toISOString()
                          .slice(0, 10)
                      : new Date().toISOString().slice(0, 10)
                  }
